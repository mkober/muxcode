...
- **Deferred to another spec** — check off, annotated `deferred to MUX-NNN` with a link to it. An open box is not free: it holds `SpecCurrentPhase` on that phase, which blocks `spec-to-pr` on work this spec has decided it will never do. The annotation is what keeps a ticked box honest — it says handed off, not done
