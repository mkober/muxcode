...
**A question-shaped dispatch is judged on the lookup, not the answer.** When the node asks *whether* something holds rather than telling you to make it hold, the requested state is "I determined the answer" — so both verdicts end `EXIT=0`, and `EXIT=1` is reserved for a lookup you could not complete. Such a node says so in its own message; obey it over the default above. Getting this backwards kills the run instead of branching it: a precheck answering `NO-PR-FOUND EXIT=1` fails its node, so the condition node that exists to route "no PR" to the commit gate never runs, and the branch the template was built around is unreachable.
