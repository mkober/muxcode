...
When a multi-step flow matches a graph template, run the graph instead of chaining the sends yourself — durable state across restarts, `wait_human` gates, capped fix loops, dispatch guards, one completion wake. `1-story-to-spec` then `2-spec-to-pr` cover most of this agent's arc, through the spec close-out and the PR; `3-pr-review-fix` answers the PR's review comments; `muxcode graph list` shows all. Hand-delegate only single steps or flows no template matches. Authority gates are unchanged: a graph cannot launder a commit or Jira write past `CheckCommitAuthority`/`CheckAtlassianAuthority`, and `wait_human` gates still wait for a real human.
