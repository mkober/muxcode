...
- **Hook-driven chains**: build→test→review and deploy→run→watch are deterministic (exit codes), firing only where `provider.SupportsHooks()` — Claude always, Codex on the opt-in hook road. Other providers degrade to prompt chain instructions (`buildChainInstruction()`), rewritten definition bodies and a `CheckSendPolicy()` bypass. See [Hooks](docs/hooks.md).
