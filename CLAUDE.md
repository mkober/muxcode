...
| `muxcode upgrade-daemons [--dry-run] [--force] [--session <name>]` | Restart running session daemons/monitors on the installed binary — a long-lived daemon otherwise keeps the code from its launch. Version-aware: each daemon records its build in `daemon.version` at startup, and one matching this binary (`Info.SameBuild` — version, commit **and** date, so a dirty rebuild still cycles) is skipped unless `--force`. `--dry-run` names both sides per session; `--session` scopes to one daemon. Kills orphan daemons whose tmux session is gone. Runs automatically on two roads: `build.sh` after `make install`, and `muxcode <dir>` when it attaches to an **already-running** session, scoped to that session alone (`MUXCODE_AUTO_UPGRADE_DAEMONS_DISABLE=1` opts out). A fresh `LaunchSession` always starts its daemon from the binary on PATH, so attaching is the only road on which one drifts behind an install — and a session is launched once but attached many times. The attach check is also the road that survives MUX-161: `build.sh`'s call runs in the build agent's sandbox where `ps` is denied, while attaching runs in the user's own terminal |
...
| `bash scripts/test-all.sh [filter]` | Runs every `scripts/test-*.sh` serially (they share scratch daemons) with a pass/fail summary; exit 1 if any fail. Skips the live-session scripts unless `MUXCODE_TEST_LIVE=1`. The `60-integration-suite` graph's single command |
| `bash scripts/test-*.sh` | Integration tests, one per feature. Each is hermetic unless noted and carries a coverage floor so a skipped section cannot report green; read the script's header for what it covers. Live-session required: `test-diff-split`, `test-hot-reload`, `test-resize-hook`. Feature tests: `test-echo-as-result`, `test-lifecycle-log-leak`, `test-branch-time-recording`, `test-disk-pressure`, `test-auto-clear`, `test-graph-orchestrator`, `test-control-pane`, `test-force-respond`, `test-send-keys-dash`, `test-prompt-mode`, `test-close-spec-guard`, `test-multi-phase-graph`, `test-graph-tui`, `test-pane-targeting`, `test-fkey-labels`, `test-verify-spec-refire`, `test-restart-definition`, `test-version`, `test-codex-hooks` (its live section runs a real codex only with `MUXCODE_CODEX_HOOKS_LIVE=1`), `test-startup-self-reply`, `test-stall-redrive-busy` (live-session; ~45s, it drives a real scratch daemon through the stall debounce), `test-status-line-task-close` (MUX-154; scratch daemon and static codex/opencode fixture panes, ~45s — a progress or rule line leaves the task in flight, the request pending for `deliver --force` and the review chain unfired; genuine replies still complete and fire `verify-spec`), `test-cancel-provenance` (MUX-182; live scratch daemon, real tmux windows and a stub agent, ~2.5 min against the installed binary — a cancelled run's worker is dead by process and window and writes nothing after the cancel, an unstoppable worker makes the cancel refuse and name a survivor that is really alive, provenance reads right on all four surfaces with the user's run created from real ancestry via `tmux run-shell`, an agent's cancel of the user's run is refused, watch notices state only exit codes, and the idle rescue never arrives as a response), `test-gate-authority` (MUX-144; live scratch daemon — an unauthorized or self-approval is refused and the gate holds, an authorized non-creator releases it, a commit dispatch with no approval is refused at the backstop via `retry --from` below an unopened gate, and both `graph-run-created` and `graph-gate-approved` name their actor). Most need an installed muxcode >= v0.1.0 |
...
- **Inbox-listener guard**: `hook guard` denies, for every role, a `muxcode inbox --poll`/`--loop` that is not the bare, lone statement of its call — `nohup`, `setsid`, a subshell, `&`, a stdout redirect, a pipe or any other statement (`CheckListenerGuard`, `bus/listener_guard.go`). The harness backgrounds the listener (`run_in_background`) and returns each message as the call's output; a detached one consumes messages into output nobody reads while receipts record them acked. On 2026-09-22 watch ran `nohup … > /tmp/watch-listener.log 2>&1 & disown`, which reparented to launchd within a second. [MUX-156](docs/requirements/backlog/MUX-156-orphaned-inbox-listener-consumes-into-the-void.md) covers orphans from any cause.
- **Injection guard**: every typed injection captures the pane first (`captureInjectionTarget`, `bus/provider.go`) — the Codex and OpenCode `SendWakeUp` roads and the Claude `SendWakeUpWithText` road alike. A bare shell prompt is refused with `ErrInjectionSkipped` plus an `injection-refused` lifecycle row: a dead agent's shell would run the payload as a command. A live Codex directory-trust prompt (`codexTrustPromptLive` — tail-anchored, because Codex draws it inline and the text stays in scrollback once accepted) is answered with Enter and the injection deferred, since a daemon relaunch runs no `AutoAccept` pass; `CodexProvider.ClassifyPane` reports it as `PaneTrustPrompt` so launch-time `AutoAccept` accepts it too. On 2026-09-09 (is-advising-gateway, a repo absent from `~/.codex/config.toml [projects]`) the banner alone read as idle, the wake sentence answered the prompt, Codex quit without persisting trust, and the relaunch loop repeated it to the restart cap. A live Codex **update** prompt (`PaneUpdatePrompt`) is answered "Skip" by `SkipCodexUpdate`, which presses Enter only on a re-capture showing Skip highlighted, and the injection deferred; it ends on the trust prompt's tail line, so `codexPromptLive` hands a shared tail to the newest prompt drawn. On 2026-09-22 (dps-data-services-pipelines) the startup wake's Enter chose the pre-selected "Update now", npm updated Codex, and build, test and review all exited to bash. A scratch pane that receives injections in an integration script must look like an agent (a `❯` stand-in), never a bare shell.
...
- **Agent wake-up**: `checkIdleAgents()` (every 5s) notifies idle agents holding actionable (request-type) messages; `LaunchSession()` wakes agents with pre-populated inboxes at startup. Self-addressed messages are dropped at `Send` (`isLoopingSelfSend`, `bus/inbox.go`); the sole exemption is the launch-time `request:startup` bootstrap (`isStartupBootstrap`) — an agent's own `response:startup` reply is correlated and never delivered or CC'd, because on 2026-09-09 a codex test agent, replying to its bootstrap with `--reply-to`, acknowledged its own acknowledgement every 5 s; `DetectMessageLoop` alerted (`loop-detected`) but is alert-only, so the drop at the source is the only stop.
...
- **Graph orchestration**: `muxcode graph run|validate|list|status|cancel|retry|approve` — a DAG control plane over the bus adding `map` fan-out, `join` barriers (`all`/`any`/`quorum`), `condition` branches (reusing `EvaluateConditions()`), capped loops (uncapped cycles fail validation) and `wait_human` gates. The daemon executes edges as ordinary bus messages, so profiles, receipts, scrub and watchdogs still apply. **Authority gates**: a commit or Atlassian node must sit downstream of a `wait_human` gate; `CheckGateApprovalAuthority` runs in the executor; default authority is the user alone, `MUXCODE_GATE_AUTHORITY_ROLES` is read from the config file only (env ignored) and sealed at daemon startup, so a mid-session edit can narrow but never widen; no agent may approve a gate on a run it created; approval is single-use, so a retry or loop re-entry needs a fresh `graph approve`. **Cancel authority** (MUX-182): an agent's `graph cancel`, or `spawn stop` of a run's worker, is refused on a run the user launched or whose creator is unrecorded/unknown — the user issues it themselves; agent-launched runs stay freely cancellable, a run already `canceling` may be finished by anyone, and `graph-run-canceled` names the actor (`CheckCancelAuthority`, `bus/cancel_authority.go`). **The runtime backstop is real for graph sends** (MUX-144 Phase 4): a dispatch carries its run and node id (`Message.GraphRun`/`GraphNode`), and `CheckCommitAuthorityForMessage` reads the **raw** sender, so `daemon` no longer normalizes to `edit` for a git-mutating action — it is judged instead on the upstream `wait_human` gate's recorded approval, which must be attributable, authorized **and** corroborated by its `graph-gate-approved` audit row (the three questions `gateApprovalHolds` asks daemon-side). A dispatch with no provenance, an unreadable run, or a hand-forged marker is refused and logged `commit-authority-refused` — the backstop fails closed, because absence of evidence that a human approved is not evidence of approval. The daemon→edit normalization itself is untouched, so replies still reach edit and non-git daemon sends are unaffected. Run state lives under `BusDir()/graphs/<run-id>/`, and the first tick after a daemon restart is the resume. Templates resolve `project > user > builtin`. **Ask before the guard, not after**: `50-spec-to-pr` puts a `phase-check` condition (`spec_phase_committable`, naming the guarded commit node) between `update-spec` and `phase-gate`; it shares `phaseCommitReady` with the `phase-progress` guard, so an open phase routes straight to `stuck-gate` and a human is asked to approve a commit only when the guard will accept it; the predicate is **anchored on HEAD's copy of the spec** (complete in the tree and not at HEAD; `specAtHEAD` strict, git failures hold the gate — MUX-183), so no run counter can re-credit a shipped phase, and "complete" means at least one item and none open — on 2026-09-09 run 1788966148 hit four stuck-gates, each preceded by a phase-gate approval the guard then withheld. The guard's own decline edge stays as the backstop. The `implement`/`fix` workers verify through the run agent (the phase's `scripts/test-*.sh`), never `go test` — the graph's test node owns the suite. **Workers**: a graph spawn worker is persistent — it answers, then idles at its prompt until the next iteration re-seeds it, and `muxcode spawn list|status` show it as `parked` (`SpawnDisplayStatus`); it is not stuck, and `graph cancel` is how work is stopped. A worker that ends before answering its seed (`spawn stop`, window gone) is **replaced** on a fresh worker with the same task (`replaceLostWorkers`, `graph-spawn-replaced`, sharing the redrive cap) — on 2026-09-09 stopping a parked implement worker seconds after its re-seed failed the run on "no live edge". Executor-owned dispatches are exempt from the daemon's idle-task watchdog (`GraphOwnsTask`): its synthetic reply at 75s beat the executor's 90s force-redrive and parked a commit node on an unverified hold; and executor redrives skip a busy pane (`graphAgentIdleFn`) — an un-receipted dispatch to a working agent whose listener died was redriven into its running turn twice. See [Architecture](docs/architecture.md#graph-orchestration-control-plane), [CLI](docs/agent-bus.md#muxcode-graph).
...
- **Daemon watchdogs** (each opt-out by env, each logging lifecycle events): long-active (`MUXCODE_ACTIVE_WATCHDOG_SECS`, 600); stuck-provider reload (`MUXCODE_STUCK_RELOAD_DISABLE=1`; cap 3/role); in-flight task expiry (`task-timeout`, 600s — expired tasks no longer block the dedup guard); permission-block (`MUXCODE_PERMBLOCK_WATCHDOG_DISABLE=1`; alert-only, stops re-waking a Claude agent wedged at a rejected permission prompt); codex-approval (`MUXCODE_CODEX_APPROVAL_WATCHDOG_DISABLE=1`, 15s — answers a read-only Codex role's command-approval prompt with Escape, the prompt's own "No"). That last one **acts** rather than alerts because the prompt is an *escalation* request — `-a on-request` is an approval policy, not a sandbox or an allowlist, so in-sandbox commands never raise it and only an attempt to work outside does — and the roles it is given to (`isReadOnlyCodexRole`: review, analyze) are told not to execute at all, so no human at the pane would ever say yes. **The approval policy is not what enforces that prohibition**; the role instructions are, and the watchdog only catches the escalation attempts that slip past them. Unlike permission-block it needs no pending inbox, since the agent parks *mid-turn* showing neither spinner nor ❯ — invisible to every other road until the 600s task timeout records the node `timed-out`, naming the clock instead of the cause (2026-09-14, is-operations-gateway: a review node burned 602s at a prompt asking to run Playwright outside its sandbox, then looped). Detection is tail-anchored (`codexApprovalPromptLive`) like the trust prompt and for the same reason — Codex draws it inline, so matching the marker alone would fire an Escape, Codex's interrupt, into every later turn. **No road re-drives a working pane**: `PaneShowsRecoverableIdle` (❯ present *and* not thinking) gates the stall watchdog, and the provider-aware `AgentIsWorking` refuses at `ForceDeliver` entry — above the unnotified-inbox branch, so an ordinary pending row is withheld mid-turn too — and inside `RedriveTask`, so `deliver --force`, receipt-gap recovery and remote inherit it. A busy pane is a **skip** (nil error, `Skipped` set), never the idle gate's error: `--force` overrides the idle-prompt gate and never this one — the injection's Escape preamble is Claude's interrupt key, and on 2026-09-09 the watchdog killed the run agent's integration script twice, 61s apart, because `PaneHasIdlePrompt` reads a composer that renders mid-turn (MUX-171). Code: `bus/stuck.go`, `bus/deliver.go`, `daemon/daemon.go`.
...
| Authority | `bus/commit_authority.go`, `bus/atlassian_authority.go`, `bus/gate_authority.go`, `bus/cancel_authority.go` |
