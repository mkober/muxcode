...
- **Keep agents deliverable (no blocking foreground commands)**: the daemon's `checkIdleAgents()` delivers/notifies **only idle** agents — an agent left **active** never receives its inbox, so messages pile up unnotified (`diagnose.go` flags this as `active-with-stale-messages`). Never run a blocking / never-exiting command (`gh pr checks --watch`, `tail -f`, log follows, interactive watchers) in an agent's own interactive pane — it wedges the agent active indefinitely. Route watch-to-completion work to the **watch** agent (log tailing) or a background `muxcode proc` (detached execution), not the foreground. Recovery for a wedged-active pane is state-dependent: (1) if the agent is merely *busy*, it returns to idle on its own and the daemon delivers; (1b) if the agent is **at the prompt but delivery is stuck** (idle misdetection, dropped send-keys/Enter, stale notified markers, parked input text), use `muxcode deliver <role> [--force]` — it injects the pending inbox via the robust text→delay→Enter→verify path, and `--force` additionally skips the idle gate, clears stale notified markers, and clears parked input. **Never hand-roll `tmux send-keys "You have new messages" Enter`** — text+Enter in one pty write is the known dropped-Enter pitfall; (2) if the TUI is genuinely **frozen** (Escape, `Ctrl-U`, **and** `Ctrl-C` all ignored, static display), every keystroke- and marker-based recovery **fails** — `agent-health --stop/--start` only toggles a marker (never touches the process), and `muxcode reload`/`GracefulStop` recover via send-keys (Escape → `/exit` → C-c) which don't land on a frozen pane. A frozen-but-alive process also passes `IsAgentAlive`, so the daemon's auto-restart (which fires only on a *dead* process) never triggers. The reliable fix is to **kill the OS process** — find the agent PID (`claude --agent <file>` under the role's pane) and `kill -TERM` (then `-KILL` if needed); the now-dead process triggers daemon auto-restart, and the fresh agent re-reads its on-disk inbox on startup. The pending message persists in the inbox across the kill. (Note: there is currently **no** single muxcode command that force-terminates a hung-but-alive agent — see `docs/requirements/backlog/delegation-message-hygiene.md`.)
...
| `watcher/watcher.go` | Bus daemon: inbox polling, trigger debounce, cron/proc/spawn/loop/compaction/ollama checks, resilience watchdogs (`checkActiveWatchdog()`, `checkStuckProviders()`, `checkTrackedTasks()` in-flight task timeout) |
