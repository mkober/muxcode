...
**Window:** every role — edit for the delegation rules; build, test and deploy for the hook-road evidence rule below; any role with an Atlassian authority limit; and all of them for the inbox-listener rule below (there is no role gate in `hookGuard` since 2026-09-22)
...
**Inbox-listener rule (every role).** `CheckListenerGuard` (`bus/listener_guard.go`) is the last check in `GuardDecisionFor` and denies any Bash call in which `muxcode inbox --poll`/`--loop` (either dash form, any path to the binary) is not the bare, lone statement. The harness backgrounds the listener itself (`run_in_background`) and returns each message as the call's output; a listener detached or redirected by the agent consumes messages into output nobody reads while receipts record them `acked` by the role — the [MUX-156](requirements/backlog/MUX-156-orphaned-inbox-listener-consumes-into-the-void.md) failure, made on purpose. On 2026-09-22 10:09:22 (session `muxcode`) watch ran `nohup muxcode inbox --poll --loop > /tmp/watch-listener.log 2>&1 & disown; sleep 1; ps aux | grep …`; pid 19757 had `PPID=1` (launchd) within a second, carrying `AGENT_ROLE=watch BUS_SESSION=muxcode`. edit verified ownership (role and session from the narrow `ps eww -p` form) and killed it; watch's inbox was empty, so nothing was lost — that time.

Detection follows **command position**. `shellWords` splits the statement (quoted spans stay one word; each parenthesis and each redirection operator with its fd prefix is a word of its own, so `--poll)` cannot hide a flag and `2>err>log` cannot hide a stdout redirect behind a stderr one), then `commandStart` peels env assignments, the grouping words `(` and `{`, and the wrapper commands `nohup`, `setsid`, `env`, `timeout`/`gtimeout`, `nice`, `command`, `exec`, `time`, `caffeinate`, `stdbuf`, `sudo` together with the option and numeric words they take (`timeout 600`, `nice -n 10`). The word left is what the statement executes. If it is `muxcode inbox` carrying `--poll`/`--loop` (either dash form, `--poll=600` included, any path to the binary), a listener is found — bare only if no wrapper was peeled and stdout is not redirected. If it is `eval`, its arguments are re-parsed as a command; if it is a shell (`sh`, `bash`, `zsh`, `dash`, `ksh`), the string after its `-c` (or `-lc`) is. A listener found either of those ways is never bare. **Every other argument is data** — `echo`'s, `grep -c`'s pattern, a bus message quoting the form — and a listener named there starts nothing. A stdout redirect is judged per operator token: `>`, `>>`, `1>`, `&>`, `>|`, `>&` deny; `2>file` and `2>&1` alone do not.

| Shape | Verdict |
|-------|---------|
| `muxcode inbox --poll --loop`, `muxcode inbox -poll 600 -loop`, `muxcode inbox --poll=600 --loop`, `./bin/muxcode inbox --loop 2>&1`, `~/.local/bin/muxcode inbox --poll --loop 2>/tmp/err` | allowed — bare; a stderr-only redirect leaves stdout with the call |
| `cd repo && muxcode inbox --poll --loop`, `MUXCODE_INBOX_POLL_TIMEOUT=600 muxcode inbox --poll --loop`, `MUXCODE_X="a b" muxcode inbox --poll --loop` | allowed — the `dropLeadingCd` prefix (shared with the evidence rule) and env assignments are peeled without counting as a wrapper |
| `nohup muxcode inbox --poll --loop`, `setsid muxcode inbox …`, `env muxcode inbox …`, `timeout 600 muxcode inbox …`, `nice -n 10 muxcode inbox …`, `(muxcode inbox --poll --loop)`, `{ muxcode inbox --poll --loop; } &`, `cd /repo && nohup muxcode inbox …` | denied — a wrapper or grouping word was peeled to reach the listener, so it is found but not bare |
| `sh -c 'muxcode inbox --poll --loop' &`, `bash -lc 'muxcode inbox --poll --loop'`, `bash -c "nohup muxcode inbox --poll --loop &"`, `eval 'muxcode inbox --poll --loop &'`, `eval muxcode inbox --poll --loop` | denied — the string a shell's `-c` or `eval` runs is a command and it runs a listener |
| `muxcode inbox --poll --loop &`, `… & disown`, `(muxcode inbox --poll --loop &)`, `(muxcode inbox --poll) &` | denied — backgrounded by the shell, not the harness |
| `muxcode inbox --poll --loop > /tmp/l.log`, `… >> log 2>&1`, `… 1>/dev/null`, `… &>log`, `… >\| log`, `… >&2`, `muxcode inbox --loop>/tmp/l`, `… 2>/tmp/err>/tmp/log` | denied — stdout redirected away from the call's output; `>&2` moves stdout to stderr, and a stderr operator cannot hide a stdout one behind it |
| `muxcode inbox --poll --loop \| tee log`, `… ; echo started`, `echo start && muxcode inbox --poll --loop` | denied — piped or bundled with any other statement |
| `echo muxcode inbox --poll --loop &`, `rg -c "muxcode inbox --poll" docs/`, `grep -c "muxcode inbox --poll" /tmp/watch-listener.log`, `ps aux \| grep -i "muxcode inbox --poll"`, `muxcode send edit notify "watch ran nohup muxcode inbox --poll --loop & disown"`, `muxcode send edit notify eval "muxcode inbox --poll --loop &"`, `muxcode memory write "listener" "never run muxcode inbox --poll --loop > log &"` | allowed — the executed word is `echo`, `rg`, `grep`, `ps` or `muxcode send`/`memory`; the listener, and even `eval`, appear only as that command's data |
| `muxcode inbox`, `muxcode inbox --peek &`, `muxcode inbox --role watch --peek > /tmp/peek.txt`, `nohup pnpm dev > log 2>&1 &`, `sh -c 'muxcode status' &`, `muxcodex inbox --poll` | allowed — no `--poll`/`--loop`, a different binary, or something else detached: not the rule's business |

The denial names the statement and says to run exactly `muxcode inbox --poll --loop` as its own call with `run_in_background=true` — no `nohup`, subshell, `&`, `disown`, `>` or pipe — and writes the usual `guard-denied` row. The positional form is the third cut of 2026-09-22: the first matched a fixed detach-word list (`nohup`, `setsid`, `(`), which `env`, `timeout`, a `{ }` group or a `sh -c` string walked straight past (review must-fix); the second matched an unquoted listener anywhere in the statement, which refused `echo muxcode inbox --poll`, an `rg -c` pattern and any bus message containing `eval` (review should-fix). Peeling a known wrapper set to the executed word is the rule that refuses every detached shape in the test table while leaving data alone. The rule binds every role because none has a use for a detached listener; it is why `hookGuard` (`cmd/hook.go`) lost its role gate and `HasAtlassianAuthorityLimit` its only caller. It refuses the one orphaning cause an agent makes deliberately; orphans from other causes (a turn-end SIGTERM that misses the child) remain MUX-156's open work. Covered by `bus/listener_guard_test.go`.
...
- **The daemon's fallback never interrupts a live turn.** When the listener does die, delivery falls to the daemon's forced road (`ForceDeliver`, the stall watchdog's re-drive, receipt-gap recovery), which on the **Claude self-poll road** types into the pane behind an Escape preamble — Claude's tool-interrupt key. That preamble is Claude-only: `SendWakeUpWithText` returns `provider.SendWakeUp` before reaching it when `SelfPollsInbox()` is false (`bus/notify.go:872–875`), and Codex hook delivery uses `guardInjection` plus `injectWakeSentence`. The **skip rule below is not scoped to Claude** — it applies on every road, because an interrupted turn is lost work whatever the provider. Those roads first ask `AgentIsWorking()`, and a working pane is skipped (`deliver-skipped-busy`, `redrive-skipped-busy`, `stall-skipped-busy`) rather than woken, because the `❯` composer renders mid-tool-call and the bare prompt is not evidence of idleness ([MUX-171](requirements/completed/MUX-171-stall-watchdog-redrive-kills-busy-claude-tool.md); see [Daemon watchdogs](architecture.md#daemon-watchdogs)).
