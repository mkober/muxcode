...
Add a GitHub contribution analytics modal triggered from the MuxCode main menu (`prefix + b → <key>`) that displays commit history, code volume, PR activity, and active days — scoped to the current repo or expanded across all repos in the user's account. Also available as a CLI command (`muxcode stats`) for programmatic use by agents.
...
#### Modal integration
- [ ] `prefix + b → <key>` opens the stats modal from the MuxCode main menu
...
| `config/tmux.conf` | Add a `"GitHub Stats"` entry to the MuxCode quick menu (`prefix + b`) — **not on `S`, which is taken**: see the key-collision note below |
...
> (`config/tmux.conf:76–94`). The menu itself is on **`prefix + b`** — it moved to `prefix + m` for a
> while and moved back on 2026-09-18, so this spec's references were swept both ways. Every
> `prefix + b → <key>` above has the key still to be chosen. Taken keys today: `n s i R w k S x e r d q X`. `g` is free and mnemonic for
...
- [ ] **Verify**: `cd tools/muxcode && go test ./...` — all tests pass
- [ ] **Verify**: `cd tools/muxcode && go vet ./...` — no issues
- [ ] **Verify**: `make install` — binary builds and installs, `prefix + b → <key>` opens stats modal
