...
- Downstream amplification: the same storm's auto-CC traffic into a non-consuming edit inbox drove the sustained `verify-spec` echo burst documented in [`verify-spec-stale-review-refire`](../drafts/MUX-007-verify-spec-stale-review-refire.md) (impact item 5) — one echo bug feeding another
