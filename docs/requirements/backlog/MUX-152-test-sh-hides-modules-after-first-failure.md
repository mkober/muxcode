...
| Spec | Relationship |
|------|--------------|
| [`MUX-153`](../drafts/MUX-153-codex-test-agent-cannot-run-the-suite.md) | Why the harness module is the one most likely to fail first on the agent meant to run the suite — its socket-binding test cannot run under the codex sandbox. That defect supplies the failure; this one hides its scope |
