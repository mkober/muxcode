...
[Ideas without specs](#ideas-without-specs). 86 delivered specs live in
...
- Once a spec's GitHub issue exists, the spec records it as a **`**Tracking:**` line directly
  ...
  [`MUX-117`](./MUX-117-pane-targeting-by-identity.md); the id conventions above cover issue
...
| 10 | [`MUX-126`](./MUX-126-edit-resume-aware-auto-restart.md) | Edit resumes without its launch flags | High | Interactive agent silently loses permission mode and tool profile; **recurred twice in one day** |
| 11 | [`MUX-110`](./MUX-110-harness-startup-tool-loop-exhaustion.md) | Harness startup message exhausts the tool loop | High | Agent burns its budget before doing work |
| 12 | [`MUX-008`](./MUX-008-unverified-daemon-auto-restart.md) | Daemon auto-restart is unverified | High | Restart reported without confirming the agent came back |
| 13 | [`MUX-009`](./MUX-009-response-echo-chain-retrigger.md) | Response echo re-triggers the chain | High | Same shape as MUX-127 Defect B on non-hook providers |
| 14 | [`MUX-122`](./MUX-122-prompt-agent-turn-attribution-and-fix.md) | Prompt-agent turn budget exhaustion | High | Stuck at 26/2/1 across four attempts; instrument built but never pointed at it |
| 15 | [`MUX-010`](./MUX-010-delegation-message-hygiene.md) | No force-terminate for a hung-but-alive agent | Medium | Recovery requires killing the OS process by hand |
| 16 | [`MUX-032`](./MUX-032-loop-detector-granularity.md) | Loop detector too coarse to act on | Medium | Governs whether the detector that flagged MUX-127 can do anything about it |
...
| MUX-117 | [`MUX-117-pane-targeting-by-identity.md`](./MUX-117-pane-targeting-by-identity.md) | High | Every path that reaches an agent resolves through a hardcoded pane index: `AgentPane()` returns the literal `"1"` for every window, and **31 further sites** — the original estimate was ~9; a verified enumeration (Phase 1) found `launcher.go` (7), `mode.go` (4), `spawn.go` (3), `reload.go` (2), `uitest_mode.go` (4), `hook.go`, `prompt_inject.go`, `control_pane.go`, `daemon/daemon.go`, plus three shell hooks — build `session:window.N` by hand, bypassing the helper entirely. tmux assigns indices **by position**, so any split before the agent renumbers it — and the failure is **silent**: keystrokes land in whatever occupies that index (an editor, a console, a git TUI where `c` commits and `P` pushes) while the system reports success. This is why `rotate-window` was deferred rather than shipped. The fix pattern already exists in-tree — the control pane identifies itself by tmux start command rather than index; generalize it to per-pane tags. **Prerequisite for [MUX-116](./MUX-116-commit-window-lazygit-diff-pane.md)** |
