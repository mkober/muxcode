...
| MUX-105 | [`MUX-105-force-respond-escalation.md`](./MUX-105-force-respond-escalation.md) | High | Daemon escalation ladder + TUI force-respond for an agent holding an un-responded request. Root cause is not a missing feature: `SendWakeUp`'s in-flight-task guard blocks the recovery injection with the very task being recovered, and returns `nil`, so `checkPollHealth` records a re-drive that never happened. Cost a live session ~20 min and two manual interventions on 2026-08-26 |
| MUX-104 | [`MUX-104-send-keys-dash-payload.md`](./MUX-104-send-keys-dash-payload.md) | High | `tmux send-keys` reads a leading `-` in the payload as a flag, so bullet-list wake-ups die with `invalid flag -` and the message is retried forever; **`-l` does not fix it, only `--` does** (verified) — two vulnerable call sites (`provider_opencode.go:213`, `notify.go:884`, the latter already carrying a misleading `-l` comment) |
