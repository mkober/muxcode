...
- [ ] `bus/provider_opencode.go` injection passes the payload after a `--` separator
- [ ] `bus/notify.go` injection passes the payload after a `--` separator, keeping `-l`
- [ ] The misleading `-l` comment at `notify.go:883` is corrected to say `-l` governs key-name interpretation and `--` is what protects a leading dash
- [ ] A payload beginning with `-` is delivered intact — the dash is not stripped and the rest of the line is not truncated
- [ ] Payloads beginning with `--` (a long flag, e.g. `--render-once`) also deliver intact
- [ ] Multi-line and unicode payloads are unaffected by the change
- [ ] A `TmuxRun` helper (or a wrapper) makes the safe form the default, so a future call site cannot re-introduce this by writing the natural-looking version
- [ ] Regression test asserts the `--` separator is present in the argv for both injection paths — argv-level, since the failure is in argument parsing and a mocked runner that only checks the payload string would pass while broken
- [ ] Negative test: a payload starting with `-` round-trips through the mocked runner without error
...
- [ ] Add `TmuxSendLiteral()` to `bus/tmux.go` emitting `send-keys -t <target> -l -- <text>`
- [ ] Switch `provider_opencode.go` text injection to it
- [ ] Switch `notify.go` text injection to it
- [ ] Correct the misleading `-l` comment in `notify.go`
- [ ] Audit the remaining `send-keys` call sites and note in code which are payload-carrying vs fixed-string

### Phase 2: Regression tests

- [ ] argv-level test: both injection paths emit `--` before the payload
- [ ] Payload starting with a single `-` round-trips without error
- [ ] Payload starting with `--` round-trips without error
- [ ] Multi-line and unicode payloads still deliver unchanged

### Phase 3: Integration test

- [ ] Create `scripts/test-send-keys-dash.sh` (hermetic — scratch tmux session, no live muxcode session needed)
- [ ] Test: bare `send-keys` with a dash-leading payload fails, pinning the underlying tmux behaviour the fix defends against
- [ ] Test: the muxcode injection path delivers a dash-leading payload into a scratch pane, verified by `capture-pane`
- [ ] Test: a `--`-leading payload delivers intact
- [ ] Test: delivery marks a receipt, so the message is consumed rather than retried forever
- [ ] Run the script and verify all checks pass

## Open questions

- **Does Codex's injection path share this?** `provider_codex.go` was not audited during this write-up; it should be checked before the fix is called complete.
- **Should `BoundWakeUpBatch` sanitise instead?** Escaping at the boundary is more robust than fixing call sites, but changes payload content; the `--` fix preserves the payload byte-for-byte and is preferred unless a case is found that `--` cannot cover.
