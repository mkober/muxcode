...
- [x] **The mirror:** a node whose work genuinely succeeded after a failed first attempt is not recorded as `failure` — see [Defect 4](#defect-4--the-mirror-a-genuine-success-recorded-as-failure) — **conditional after 15:40**: with the agent's `EXIT=0` beside the stale failure row the node now **holds** (`graph-outcome-conflict`) instead of failing — the incident's three fix laps and eight stuck-gates become one approval; with the sentinel omitted the row alone still records `failure`. `TestExecSendOutcomeHoldsOnContradiction` pins the first case — **unchanged by attribution (2026-09-15)**: the stale failure row is a test command, so it *does* testify for a `test` action, and only the agent's token can say the unrecognised re-run passed. Two ways to close, the user's call (put to them 2026-09-15): accept the residual — the test role is instructed to emit the token, omission is that agent's defect, and the conflict rule holds the moment it is emitted — or hold a failure row that has no corroborating token, which trades the mirror for a hold on every tokenless genuine failure — **closed 2026-09-18: residual accepted by the user** ("close the spec", said after plan laid out that accepting the residual is what closes it). The tokenless case still records `failure`; that is a known, accepted limit, not a fixed defect. Phase 5's mirror test pins the token-present case and its negative control
...
- [x] Add a commit step between `c` and `d` in `commit-pr-review-loop`, or restructure so `d` has a citable sha — **and give `c` a verdict (2026-09-15):** with attribution live, `c` (`edit:edit`) has no command that could evidence it and edit emits no token (`code-editor.md` has no `EXIT=` line; send-road dispatches seed none), so it holds on every run — the restructure must seed the token in the dispatch as `spawnVerdictInstruction` does, add the instruction to the definition, or move the work to a spawn node — **done 2026-09-16 (edit; verified by plan against the working tree):** `push-fixes` (`commit:commit`, `graph_templates.go:86`) sits between `c` and `d` under `gate2`, whose approval text now names the commit and push; it stages and commits the review-feedback changes, pushes to the PR branch and reports the sha, and `d` is told to cite that sha per fix. `c`'s verdict is the seeded token: `seedVerdictToken` (`graph_exec.go:756`) appends `verdictTokenInstruction` (`:738` — `spawnVerdictInstruction` renamed, now the one seed both roads use) to every send dispatch whose action is in `actionsWithoutCommandEvidence`, which closes the class, not the instance — no template can forget what it never had to remember. Pinned by `TestCommitPrReviewLoopCommitsFixesBeforeReplying`, `TestSeedVerdictToken` (six unevidenced actions seeded with the message intact ahead of the seed; actions a command evidences left alone as the negative control) and `TestExecSendSeedsVerdictForUnevidencedAction` (the executor's `edit:edit` dispatch carries the instruction, the following `build` dispatch does not)
- [x] Confirm the new node is gated per the commit-authority rules (a commit node must be downstream of a `wait_human`) — since MUX-144 this is enforced at runtime by `checkGraphCommitDispatch`, which walks `gateTerritory` for a covering gate and re-checks its approval; an inserted node falls in **`gate2`**'s territory, so verify `gate2`'s single approval legitimately covers both `c` and the new commit node — **confirmed 2026-09-16:** the validator's gate rule accepts the template (`push-fixes` is downstream of `gate2`), `TestCommitPrReviewLoopCommitsFixesBeforeReplying` asserts `nodeRequiresGate(push-fixes)` so the runtime backstop (`checkGraphCommitDispatch`) will judge it on `gate2`'s recorded approval, and `gate2`'s message was rewritten to say what that approval releases — *addressing the feedback, then committing and pushing those fixes, and replying* — so the one approval covers what it names. The runtime path has not been exercised live
- [x] Verify `graph validate` still passes for the amended template — **2026-09-16:** `muxcode graph validate commit-pr-review-loop` → OK, 14 nodes, 15 edges (run by plan); `TestBuiltinGraphTemplatesValidate` green in the 16:28:38 suite
- [x] Check the other six builtin templates for the same shape — a node asked to cite work that nothing has committed — **scanned 2026-09-16 (edit), one hit: `story-to-spec`**, recorded below; whether it is fixed is the user's call, so it is a finding and not a box

**Finding — `story-to-spec` has the same shape** (2026-09-16, edit's scan; verified by plan against the
working tree). `draft` (`plan:update-docs`) creates `docs/requirements/drafts/<ID>-<slug>.md` and sets
it active; `update-gate` then releases `jira-update` (`plan:jira-write`) and `issue-update`
(`commit:issue-update`), each told to reference *"the new requirements doc"* — and nothing between
`draft` and the tracker write commits or pushes it, so the tracker is pointed at a file that exists in
one working tree only. Same class as `c`→`d`: a node asked to cite work nothing has committed.
`graph validate story-to-spec` passes (7 nodes, 6 edges) because the gate rule is about authority, not
about citable state. **Not fixed here — the user decides.** The fix has `push-fixes`' shape: a
`commit:commit` node between `draft` and the tracker writes, under `update-gate` with its message
naming the commit — or the tracker text saying the doc is on an unpushed branch.
...
**Complete — 2026-09-18 (spec 56/56; acceptance 13/13; all five phases closed).** Closed on the user's
instruction. The last box, the mirror criterion, closed by the user **accepting the residual**: with
the agent's verdict token present a contradicted node holds (`graph-outcome-conflict`) instead of
failing; with the token omitted the stale row still records `failure`, and that is accepted, not
fixed. One Phase 3 step is ticked as **deferred to MUX-185, not done** — actor provenance is that
spec's work. Moved to `completed/` 2026-09-18 on the user's instruction, relayed through edit; the
backlog index lost this spec's defects and category rows and gained its registry row. The dated entries below are the working record, oldest first.
...
**Phase 4 verified 2026-09-16 16:45 by plan (edit's `update-docs`; no dispatch named the branch) —
4/4** (spec 43/56; acceptance 10/13). HEAD moved to `877d08a`: `009d3aa` committed the attribution
work, `ab3718a` the near-miss launcher guard, `8789ea8` the `pr-precheck` skip, `21fa3ee` the
question-shaped `EXIT` override and `877d08a` the docs — so yesterday's conditional ticks now stand on
committed code. **In the tree, uncommitted:** `push-fixes` (`commit:commit`, `graph_templates.go:86`)
between `c` and `d` under `gate2`, whose approval text now names the commit and push, with `d` told to
cite the sha reported upstream; `seedVerdictToken` (`graph_exec.go:756`) appending
`verdictTokenInstruction` (`:738`, the renamed `spawnVerdictInstruction`) to every send dispatch whose
action is in `actionsWithoutCommandEvidence`, applied at `:1027`; three tests —
`TestCommitPrReviewLoopCommitsFixesBeforeReplying`, `TestSeedVerdictToken`,
`TestExecSendSeedsVerdictForUnevidencedAction` — plus a spawn-seed assertion added to
`TestExecSpawnTaskUnprefixedWithoutSendNodes`, because its prefix check passed with or without the
seed. **Evidence:** suite hook-observed exit 0 at 16:28:38, 3012 pass / 0 fail / 2 skip, after two
failing runs (16:25:01, 16:26:49 — `TestExecSendSeedsVerdictForUnevidencedAction` tripped first on
the `graph-run-created` event in edit's inbox, then on its own negative control; fixed in the fixture,
`completeSendNodeWithReply` now replies from an away-from-edit sender); review 16:29:09 *0 must-fix,
0 should-fix*, nits on comment placement, answered by a 16:29:28 test-file edit not re-run since;
`muxcode graph validate` OK on `commit-pr-review-loop` (14 nodes, 15 edges) and `story-to-spec`
(7, 6), run by plan. **Ticked:** all four Phase 4 steps, and the Phase 3 consequence (`c` holds every
run) closed by the seeding. **Left open:** criterion 7 — structurally met, closes on a run; the
`story-to-spec` finding under Phase 4 is the user's call and deliberately not a box. **Daemon:**
`daemon.version` still reads yesterday's `97b7288-dirty` build of 13:37 — none of today's five commits
