...
- [ ] The never-author and doc-file guards hold for codex build/test/review — [MUX-157](../backlog/MUX-157-role-boundary-an-agent-can-ignore.md)'s missing road — *doc-file holds on every patch path; the never-author rule set does not exist in `guardRulesForRole` yet (MUX-157's to add — it will apply to both providers unchanged)*
