...
| `MUXCODE_CODEX_APPROVAL_WATCHDOG_DISABLE` | (unset) | Set to `1` to disable the codex-approval watchdog — the daemon's 15s sweep that answers a **read-only Codex role's** (`review`, `analyze`) command-approval prompt with Escape, the prompt's own "No", so the agent does not park mid-turn until the 600s task timeout records its node `timed-out`. The answer is fixed by configuration: `-a on-request` is given only to roles that must not execute, so it cannot be yes. See [Daemon watchdogs](architecture.md#daemon-watchdogs) |
