...
| `MUXCODE_CONTROL_PANE_HEIGHT` | `18` | Pane height in rows, re-applied after every window refit |
| `MUXCODE_CONTROL_PANE_SURFACE` | `runs` | Starting surface: `runs`, `gates`, `prompt`, or `launcher`/`templates`. An unknown name degrades to the graph run list rather than an empty pane |
...
| `MUXCODE_PERMBLOCK_WATCHDOG_DISABLE` | (unset) | Set to `1` to disable the permission-block watchdog — alert-only; it stops re-waking a Claude agent wedged at a rejected permission prompt and sends edit one `permission-blocked` event |
| `MUXCODE_CODEX_APPROVAL_WATCHDOG_DISABLE` | (unset) | Set to `1` to disable the codex-approval watchdog — the daemon's 15s sweep that answers a **read-only Codex role's** (`review`, `analyze`) escalation prompt — Codex asking to run a command past its sandbox — with Escape, the prompt's own "No", so the agent does not park mid-turn until the 600s task timeout records its node `timed-out`. `-a on-request` is an approval *policy* ("the model decides when to ask"), not a sandbox or an allowlist: an in-sandbox command runs unprompted, and the role's no-execution rule is enforced by its instructions, not by this flag. The answer is fixed because a read-only role is never granted escalation. See [Daemon watchdogs](architecture.md#daemon-watchdogs) |
...
Per-branch active working time, accumulated by the daemon and surfaced in the tmux status bar, `muxcode branch-time --status`, and the `prepare-commit-msg` commit trailer. See the [branch-time-tracking spec](requirements/completed/MUX-040-branch-time-tracking.md).
