...
**Codex self-update prompt (2026-09-22)**: when a newer release exists, Codex draws "✨ Update available! 0.155.0 -> 0.155.1" under the banner with three options — "1. Update now" pre-selected, "2. Skip", "3. Skip until next version" — ending on the trust prompt's own tail line, "Press enter to continue". The box-drawing test read it as `PaneIdle`: in `dps-data-services-pipelines` at 10:11:27 `AutoAccept` logged `agent-ready`, the startup wake typed its sentence plus Enter at 10:11:28, Enter chose *Update now*, `npm install -g @openai/codex` ran, Codex exited to bash, and build, test and review all died; "You have new messages" then surfaced in bash as `-bash: You: command not found` — consistent with the same wake arriving through the tty buffer after the exit (no separate injection row), not proven. Evidence: `~/.config/muxcode/logs/snapshots/dps-data-services-pipelines-{build,test,review}-1790086342/`. `ClassifyPane` now returns `PaneUpdatePrompt` for a live prompt (`codexUpdatePromptLive`), checked right after the trust prompt; `AcceptStartup`, the mode-cycling accept loop and `guardInjection` all answer it with `SkipCodexUpdate`, which chooses **"2. Skip"** — not *Update now* (installs software the operator did not ask for and exits) and not *Skip until next version* (persists a preference in the user's own `~/.codex` config). It moves the highlight (`›`) with Down/Up, re-captures after each move, presses Enter **only** on a capture that shows Skip highlighted, and gives up after two moves: an unreadable or unmoving highlight presses nothing and logs `auto-accept`/`update-skip-failed` (`warn`), because on any other frame Enter is the install. Success logs `auto-accept`/`update-prompt` (or `mode-accept`/`update-prompt`), and on the injection road the wake is deferred with `ErrInjectionSkipped`. Because the update and trust prompts end on the same line, the three inline detectors now share `codexPromptLive(content, marker, tail, window)`, which requires that no other Codex prompt marker was drawn **after** the one being tested — an accepted trust prompt in scrollback can no longer claim the update prompt beneath it (and answer it with Enter), nor the reverse. Pinned by `bus/codex_update_prompt_test.go`.
...
- As graph workers (`50-spec-to-pr`'s `implement` and `fix` nodes), verify a phase through the **run agent** — `muxcode send run run "bash scripts/test-<feature>.sh" --wait` — and quote its counts and task id before reporting; never `go test`, the graph's test node owns the suite. Plan's verify credits the run agent's store row, not the worker's report (MUX-167)
...
with Enter (an `auto-accept`/`trust-prompt` row) and the injection deferred, and a live Codex
update prompt is answered with Skip (`SkipCodexUpdate`, an `auto-accept`/`update-prompt` row —
or `update-skip-failed` and nothing pressed when Skip cannot be shown highlighted) and the
injection deferred likewise, since the Enter a wake-up ends with would otherwise choose the
pre-selected *Update now* (see [Codex self-update prompt](#multi-cli-provider-support)). `deliver --force`
