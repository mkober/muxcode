...
- As graph workers (`2-spec-to-pr`'s `implement` and `fix` nodes), verify a phase through the **run agent** — `muxcode send run run "bash scripts/test-<feature>.sh" --wait` — and quote its counts and task id before reporting; never `go test`, the graph's test node owns the suite. Plan's verify credits the run agent's store row, not the worker's report (MUX-167)
