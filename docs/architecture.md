...
4. `send` calls `Notify()` to alert the recipient (provider-aware):
   ...
   - **Non-hook providers** (OpenCode, Codex CLI, local LLM): routed directly to `provider.SendWakeUp()` — OpenCode and Codex inject message payload via send-keys (self-addressed messages filtered to prevent echo loops — `isLoopingSelfSend` in `bus/inbox.go` refuses every self-send at `Send` except the launch-time bootstrap, and that exemption is keyed on type **and** action (`isStartupBootstrap`: `request:startup`); an agent's own `response:startup` is correlated through `recordUndeliveredReply` and never delivered or CC'd. On 2026-09-09 a codex test agent that answered its bootstrap with `--reply-to` acknowledged its own acknowledgement every 5 s — 16 rows in a minute — a one-role echo `DetectMessageLoop` reports but cannot break: [`MUX-169`](requirements/drafts/MUX-169-startup-self-reply-echo-loop.md)), local LLM is no-op
