...
### 2026-04-29 16:50
Completed all 5 phases of opencode-deepseek-editor feature. Phase 1: Edit tool profile with DenyTools for delegation enforcement. Phase 2: OpenCode provider integration (translateToolProfile, adaptBodyForNonHookProvider, model defaults). Phase 3: Provider-aware SharedPrompt (compact instructions, manual bus messaging for edit). Phase 4: Daemon checkNonHookEdits() for non-hook workflow state transitions. Phase 5: Integration test scripts/test-opencode-edit.sh (25/25 pass) + muxcode agent config subcommand in cmd/agent.go. Also added MUXCODE_EDIT_CLI=opencode and MUXCODE_EDIT_MODEL=opencode-go/deepseek-v4-pro to both ~/.config/muxcode/config and .muxcode/config. All changes uncommitted on main. Build passes, all Go tests pass, integration test passes, review approved.
