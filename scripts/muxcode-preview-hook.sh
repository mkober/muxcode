...
if [ -f "$TEMP_FILE" ]; then
  ...
  sleep 0.1
  tmux send-keys -t "$SESSION:edit.0" ":exe 'sil! b!'.get(g:,'_mux_buf',bufnr()) | sil! diffoff! | sil! only | setlocal number" Enter
  sleep 0.2
  rm -f "$TEMP_FILE"
fi
...
tmux send-keys -t "$SESSION:edit.0" Escape Escape
sleep 0.1
tmux send-keys -t "$SESSION:edit.0" ":exe 'sil! e! +$LINE $ESCAPED_PATH' | setlocal foldlevel=99 number" Enter
