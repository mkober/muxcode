#!/bin/bash
# muxcoder-bash-hook.sh — PostToolUse hook for Bash commands
#
...
case "$COMMAND" in
  muxcoder-agent-bus*|agent-bus*) exit 0 ;;
esac
...
BUILD_PATTERNS="${MUXCODER_BUILD_PATTERNS:-./build.sh|pnpm*build|go*build|make|cargo*build|cdk*synth|tsc}"
TEST_PATTERNS="${MUXCODER_TEST_PATTERNS:-./test.sh|jest|pnpm*test|pytest|go*test|go*vet|cargo*test|vitest}"
...
if [ "$is_build" -eq 1 ]; then
  if [ -z "$EXIT_CODE" ]; then
    muxcoder-agent-bus send analyze notify "Build completed (exit code unknown): $COMMAND" --type event --no-notify
  elif [ "$EXIT_CODE" = "0" ]; then
    muxcoder-agent-bus send analyze notify "Build succeeded: $COMMAND" --type event --no-notify
    muxcoder-agent-bus send test test "Build succeeded — run tests and report results" --type request
  else
    muxcoder-agent-bus send analyze notify "Build FAILED (exit $EXIT_CODE): $COMMAND" --type event --no-notify
    muxcoder-agent-bus send edit notify \
...
if [ "$is_test" -eq 1 ]; then
  if [ -z "$EXIT_CODE" ]; then
    muxcoder-agent-bus send analyze notify "Tests completed (exit code unknown): $COMMAND" --type event --no-notify
  elif [ "$EXIT_CODE" = "0" ]; then
    muxcoder-agent-bus send analyze notify "Tests passed: $COMMAND" --type event --no-notify
    muxcoder-agent-bus send review review "Tests passed — review the changes and report results to edit" --type request
  else
    muxcoder-agent-bus send analyze notify "Tests FAILED (exit $EXIT_CODE): $COMMAND" --type event --no-notify
    muxcoder-agent-bus send edit notify \
