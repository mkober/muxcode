#!/bin/bash
# muxcode-deploy-log.sh - Poll deploy history every N seconds
# Used in the deploy window's left pane during muxcode sessions
#
# Usage: muxcode-deploy-log.sh [interval_seconds]

set -uo pipefail

INTERVAL="${1:-5}"
SESSION="${BUS_SESSION:-$(tmux display-message -p '#S' 2>/dev/null || echo default)}"
HISTORY_FILE="/tmp/muxcode-bus-${SESSION}/deploy-history.jsonl"
...
while true; do
  BUF=""
  BUF+="${PURPLE}  deploy log${RESET}  ${DIM}$(date '+%H:%M:%S')${RESET}  ${DIM}(every ${INTERVAL}s)${RESET}\n"
  ...
  # Parse deploy history with jq (python3 fallback)
  if command -v jq &>/dev/null; then
    ...
    # Last deploy summary (output from most recent deploy)
