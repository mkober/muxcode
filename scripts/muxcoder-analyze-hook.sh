#!/bin/bash
# muxcoder-analyze-hook.sh — PostToolUse hook for Write/Edit/NotebookEdit
#
...
BUS_DIR="/tmp/muxcoder-bus-${SESSION}"
if [ -d "$BUS_DIR" ]; then
  export BUS_SESSION="$SESSION"
  export AGENT_ROLE="$(tmux display-message -p '#W' 2>/dev/null || echo 'edit')"

  # Configurable routing rules
  ROUTE_RULES="${MUXCODER_ROUTE_RULES:-test|spec=test cdk|stack|construct|terraform|pulumi=deploy .ts|.js|.py|.go|.rs=build}"
  ...
  for rule in "${RULES[@]}"; do
    ...
    for pat in "${PATS[@]}"; do
      if [[ "$FILE_PATH" == *"$pat"* ]]; then
        muxcoder-agent-bus send "$local_target" notify "File changed: $FILE_PATH" --type event --no-notify
        routed=1
        break 2
      fi
    done
