...
while true; do
  ...
  BUF=""
  BUF+="${PAD}${PURPLE}Test${RESET}  ${DIM}$(date '+%H:%M:%S')${RESET}  ${DIM}(every ${INTERVAL}s)${RESET}\n"
  BUF+="${PAD}${DIM}$(printf '%.0s─' $(seq 1 "$SEP_WIDTH"))${RESET}\n"
  BUF+="\n"

  if [ ! -f "$HISTORY_FILE" ] || [ ! -s "$HISTORY_FILE" ]; then
    BUF+="${PAD}${DIM}no tests yet${RESET}\n"
    printf '\033[2J\033[H'
    echo -ne "$BUF"
    sleep "$INTERVAL"
    continue
  ...
  if command -v jq &>/dev/null; then
    ...
    BUF+="${PAD}${CYAN}recent tests${RESET}\n"
    ...
    if [ -n "$DISPLAY_OUTPUT" ] || [ "$LAST_EC" != "0" ]; then
      if [ "$LAST_EC" = "0" ]; then
        BUF+="${PAD}${GREEN}⏺ Tests passed${RESET}  ${DIM}${LAST_TIME}${RESET}\n\n"
      else
        LAST_CMD=$(jq -s -r '.[-1].command // ""' "$HISTORY_FILE" 2>/dev/null)
        BUF+="${PAD}${RED}⏺ Tests failed${RESET}  ${DIM}${LAST_TIME}${RESET}\n"
      ...
      if [ "$LAST_EC" = "0" ]; then
        BUF+="${CONT_PAD}${DIM}- All tests passed${RESET}\n"
      fi
      BUF+="\n"
    fi
        ...
        LAST_DISPLAY_LINE=*)
          if [ "$HAS_DISPLAY" -eq 0 ]; then
            ...
            if [ "$PY_LAST_EC" = "0" ]; then
              BUF+="${PAD}${GREEN}⏺ Tests passed${RESET}  ${DIM}${PY_LAST_TIME}${RESET}\n\n"
            else
              BUF+="${PAD}${RED}⏺ Tests failed${RESET}  ${DIM}${PY_LAST_TIME}${RESET}\n"
