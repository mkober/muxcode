...
func ClassifyCommand(command string) CommandType {
	// Skip bus commands
	if strings.HasPrefix(command, "muxcode-agent-bus") || strings.HasPrefix(command, "agent-bus") {
		return CmdBus
	}
...
var editGuardRules = []guardRule{
	{
		...
		reason:   `BLOCKED: PR read commands are prohibited in the edit window. Delegate to the commit agent. Run this command: muxcode-agent-bus send commit pr-read "Read the PR on the current branch and report review feedback, CI failures, and suggested fixes" --wait`,
	},
	{
		prefixes: []string{"gh pr create", "gh pr merge", "gh pr close", "gh pr reopen", "gh pr edit", "gh release"},
		reason:   `BLOCKED: PR/release mutations are prohibited in the edit window. Delegate to the commit agent. Run: muxcode-agent-bus send commit commit "<describe the PR/release operation>" --wait`,
	},
	{
		prefixes: []string{"gh "},
		reason:   `BLOCKED: gh commands are prohibited in the edit window. Delegate to the commit agent. Run: muxcode-agent-bus send commit commit "<describe the operation>" --wait`,
	},
	{
		prefixes: []string{"git "},
		reason:   `BLOCKED: All git commands are prohibited in the edit window. Delegate to the commit agent. Run: muxcode-agent-bus send commit commit "<describe the git operation>" --wait`,
	},
	{
		prefixes: []string{"./build.sh", "pnpm build", "pnpm run build", "npm run build", "go build", "cargo build", "tsc "},
		reason:   `BLOCKED: Build commands are prohibited in the edit window. Delegate to the build agent. Run: muxcode-agent-bus send build build "Run ./build.sh and report results" --wait`,
	},
	{
	...
	},
	{
		...
		reason: `BLOCKED: Test commands are prohibited in the edit window. Delegate to the test agent. Run: muxcode-agent-bus send test test "Run tests and report results" --wait`,
	},
	{
		prefixes: []string{"cdk ", "npx cdk ", "terraform ", "pulumi ", "sam "},
		reason:   `BLOCKED: Deploy commands are prohibited in the edit window. Delegate to the deploy agent. Run: muxcode-agent-bus send deploy deploy "<describe the deploy operation>" --wait`,
	},
	{
		prefixes: []string{"aws logs", "tail -f", "tail -F", "kubectl logs", "docker logs", "docker-compose logs", "stern "},
		reason:   `BLOCKED: Log tailing commands are prohibited in the edit window. Delegate to the watch agent. Run: muxcode-agent-bus send watch watch "<describe what logs to tail>" --wait`,
	},
}
...
func ShouldPollInbox(command string) bool {
	if !strings.HasPrefix(command, "muxcode-agent-bus send ") && !strings.HasPrefix(command, "agent-bus send ") {
		return false
	}
...
func PollInbox(session string, timeout, pollInterval time.Duration) string {
	...
	return fmt.Sprintf("No response received within %ds. Check manually: muxcode-agent-bus inbox --peek", int(timeout.Seconds()))
}
