...
// Usage: muxcode chain <event_type> <outcome> [--exit-code N] [--command CMD] [--no-notify] [--dry-run]
// Exit codes: 0 = sent, 1 = error, 2 = no chain configured
func Chain(args []string) {
	if len(args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode chain <event_type> <outcome> [--exit-code N] [--command CMD] [--no-notify] [--dry-run]\n")
		os.Exit(1)
	}
