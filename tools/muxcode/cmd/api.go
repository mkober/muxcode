...
// Api handles the "muxcode api" subcommand.
func Api(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api <env|collection|history|import> [args...]\n")
		os.Exit(1)
	}
...
func apiEnv(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api env <list|get|create|set|delete> [args...]\n")
		os.Exit(1)
	}
	...
	case "get":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api env get <name>\n")
			os.Exit(1)
		}
		apiEnvGet(args[1])
	case "create":
	...
	case "set":
		if len(args) < 4 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api env set <name> <key> <value>\n")
			os.Exit(1)
		}
		apiEnvSet(args[1], args[2], strings.Join(args[3:], " "))
	case "delete":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api env delete <name>\n")
			os.Exit(1)
		}
		apiEnvDelete(args[1])
	default:
...
func apiEnvCreate(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api env create <name> --base-url <url>\n")
		os.Exit(1)
	}
...
func apiCollection(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api collection <list|get|create|delete|add-request|remove-request> [args...]\n")
		os.Exit(1)
	}
	...
	case "get":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api collection get <name>\n")
			os.Exit(1)
		}
	...
	case "delete":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api collection delete <name>\n")
			os.Exit(1)
		}
	...
	case "remove-request":
		if len(args) < 3 {
			fmt.Fprintf(os.Stderr, "Usage: muxcode api collection remove-request <collection> <name>\n")
			os.Exit(1)
		}
...
func apiCollectionCreate(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api collection create <name> [--description desc] [--base-url url]\n")
		os.Exit(1)
	}
...
func apiCollectionAddRequest(args []string) {
	if len(args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api collection add-request <collection> <name> --method METHOD --path PATH [--header key:value] [--body json] [--query key=value]\n")
		os.Exit(1)
	}
...
func apiHistory(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "Unknown flag: %s\n", args[i])
			fmt.Fprintf(os.Stderr, "Usage: muxcode api history [--collection name] [--limit N]\n")
			os.Exit(1)
		}
	}
...
func apiImport(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode api import <source-dir>\n")
		os.Exit(1)
	}
