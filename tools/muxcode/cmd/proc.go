...
// Proc handles the "muxcode proc" subcommand.
func Proc(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode proc <start|list|status|log|stop|clean> [args...]\n")
		os.Exit(1)
	}
...
func procStart(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode proc start \"<command>\" [--dir DIR]\n")
		os.Exit(1)
	}
...
func procList(args []string) {
	...
	for _, arg := range args {
		...
		default:
			...
			fmt.Fprintf(os.Stderr, "Usage: muxcode proc list [--all]\n")
			os.Exit(1)
		}
	}
...
func procStatus(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode proc status <id>\n")
		os.Exit(1)
	}
...
func procLog(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode proc log <id> [--tail N]\n")
		os.Exit(1)
	}
...
func procStop(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode proc stop <id>\n")
		os.Exit(1)
	}
