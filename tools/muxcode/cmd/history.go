...
// History handles the "muxcode history" subcommand.
// Usage: muxcode history <role> [--limit N] [--context]
func History(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode history <role> [--limit N] [--context]\n")
		os.Exit(1)
	}
