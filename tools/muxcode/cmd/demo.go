...
// Demo handles the "muxcode demo" subcommand.
func Demo(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode demo <run|list> [args...]\n")
		os.Exit(1)
	}
...
func demoRun(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			if args[i][0] == '-' {
				...
				fmt.Fprintf(os.Stderr, "Usage: muxcode demo run [SCENARIO] [--speed FACTOR] [--dry-run] [--no-switch]\n")
				os.Exit(1)
			}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'muxcode demo list' to see available scenarios.\n")
		os.Exit(1)
	}
