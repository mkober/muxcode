...
// AgentHealth handles the "muxcode agent-health" subcommand.
// Usage: muxcode agent-health [--stop <role>] [--start <role>] [--check <role>]
func AgentHealth(args []string) {
	session := bus.BusSession()

	if len(args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode agent-health [--stop <role>] [--start <role>] [--check <role>]\n")
		os.Exit(1)
	}
