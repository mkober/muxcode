...
// Spawn handles the "muxcode spawn" subcommand.
...
func spawnList(args []string) {
	...
	for _, arg := range args {
		...
		default:
			...
			fmt.Fprintf(os.Stderr, "Usage: muxcode spawn list [--all]\n")
			os.Exit(1)
		}
	}
...
func spawnStatus(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode spawn status <id>\n")
		os.Exit(1)
	}
...
func spawnResult(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode spawn result <id>\n")
		os.Exit(1)
	}
...
func spawnStop(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode spawn stop <id>\n")
		os.Exit(1)
	}
