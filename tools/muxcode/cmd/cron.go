...
// Cron handles the "muxcode cron" subcommand.
func Cron(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode cron <add|list|remove|enable|disable|history> [args...]\n")
		os.Exit(1)
	}
...
func cronAdd(args []string) {
	if len(args) < 4 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode cron add <schedule> <target> <action> <message>\n")
...
func cronList(args []string) {
	...
	for _, arg := range args {
		...
		default:
			...
			fmt.Fprintf(os.Stderr, "Usage: muxcode cron list [--all]\n")
			os.Exit(1)
		}
	}
...
func cronRemove(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode cron remove <id>\n")
		os.Exit(1)
	}
...
func cronEnable(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode cron enable <id>\n")
		os.Exit(1)
	}
...
func cronDisable(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode cron disable <id>\n")
		os.Exit(1)
	}
...
func cronHistory(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "Unknown flag: %s\n", args[i])
			fmt.Fprintf(os.Stderr, "Usage: muxcode cron history [--id CRON_ID] [--limit N]\n")
			os.Exit(1)
		}
	}
