...
// Guard handles the "muxcode guard" subcommand.
// Usage: muxcode guard [role] [--json] [--threshold N] [--window N]
	...
	for i := 0; i < len(remaining); i++ {
		...
		default:
			if remaining[i][0] == '-' {
				...
				fmt.Fprintf(os.Stderr, "Usage: muxcode guard [role] [--json] [--threshold N] [--window N]\n")
				os.Exit(1)
			}
			role = remaining[i]
		}
