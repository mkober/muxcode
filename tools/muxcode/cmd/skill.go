...
// Skill handles the "muxcode skill" subcommand.
func Skill(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode skill <list|load|search|create|prompt> [args...]\n")
		os.Exit(1)
	}
...
func skillList(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			...
			fmt.Fprintf(os.Stderr, "Usage: muxcode skill list [--role ROLE]\n")
			os.Exit(1)
		}
	}
...
func skillLoad(args []string) {
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode skill load <name>\n")
		os.Exit(1)
	}
...
func skillSearch(args []string) {
	...
	if query == "" {
		fmt.Fprintf(os.Stderr, "Usage: muxcode skill search <query> [--role ROLE]\n")
		os.Exit(1)
	}
...
func skillCreate(args []string) {
	...
	if len(positional) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: muxcode skill create <name> <desc> [--roles r1,r2] [--tags t1,t2] [body]\n")
		os.Exit(1)
	}
