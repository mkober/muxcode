...
var lifecycleUsage = `Usage: muxcode lifecycle <subcommand> [args...]
...
// Lifecycle handles the "muxcode lifecycle" subcommand.
...
func lifecycleLog(args []string) {
	if len(args) < 4 {
		fmt.Fprintln(os.Stderr, "Usage: muxcode lifecycle log <session> <level> <source> <event> [--detail TEXT] [--pid N]")
		os.Exit(1)
	}
