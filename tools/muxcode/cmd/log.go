...
// Log handles the "muxcode log" subcommand.
// Usage: muxcode log <role> "<summary>" [--exit-code N] [--command CMD] [--output TEXT] [--output-stdin] [--output-file PATH]
//
...
func runLog(args []string, stdin io.Reader) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: muxcode log <role> \"<summary>\" [--exit-code N] [--command CMD] [--output TEXT] [--output-stdin] [--output-file PATH]")
	}
