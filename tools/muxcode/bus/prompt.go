...
func SharedPrompt(role string) string {
	...
	b.WriteString("**When to compact**: Compact proactively to avoid running out of context. Triggers:\n")
	b.WriteString("- After completing a multi-step task (PR creation, rebase, deploy, review)\n")
	b.WriteString("- After 3+ consecutive requests without compacting\n")
	b.WriteString("- When you receive a `compact-recommended` alert from the daemon\n")
