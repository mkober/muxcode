...
`
	...
	if claude.Models[0] != "claude-opus-5-5" {
		t.Errorf("claude models[0] = %q, want %q", claude.Models[0], "claude-opus-5-5")
	}
...
`
	...
	if claude.Disabled[0] != "claude-opus-5-5" {
		t.Errorf("disabled[0] = %q, want %q", claude.Disabled[0], "claude-opus-5-5")
	}
}
...
func TestAddModel(t *testing.T) {
	...
	// Add new model
	if !AddModel(cfg, "claude", "claude-opus-5-5") {
...
func TestAddModel_FromDisabled(t *testing.T) {
	cfg := &ModelConfig{Providers: map[string]ProviderModels{
		"claude": {
			Models:   []string{"claude-sonnet-5"},
			Disabled: []string{"claude-opus-5-5"},
		},
	}}
...
func TestRemoveModel(t *testing.T) {
	cfg := &ModelConfig{Providers: map[string]ProviderModels{
		"claude": {
			Default: "claude-sonnet-5",
			Models:  []string{"claude-opus-5-5", "claude-sonnet-5"},
		},
	}}

	if !RemoveModel(cfg, "claude", "claude-opus-5-5") {
...
func TestSetDefaultModel(t *testing.T) {
	...
	if !SetDefaultModel(cfg, "claude", "claude-opus-5-5") {
		t.Error("SetDefaultModel returned false for enabled model")
	}
	if cfg.Providers["claude"].Default != "claude-opus-5-5" {
		t.Errorf("default = %q, want %q", cfg.Providers["claude"].Default, "claude-opus-5-5")
	}
...
func TestFormatModelList(t *testing.T) {
	...
	if !strings.Contains(output, "claude-opus-5-5") {
