...
// panes, so the active agent always sits in the host window's pane 1 —
// the target is the window itself, and ActiveModeRole names who is
// actually there (a window cycled to another mode must not have the
...
func InjectPromptText(session, window, text string) (string, error) {
	...
	target := session + ":" + window + "." + AgentPane(window)
